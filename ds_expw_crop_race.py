...
class DatasetEXPWRACECROP(Dataset):
    def __init__(self, 
                 train = True, 
                 transform=None,
        ...
        if decision_val == 'race': # decision is based on race
            try:
                ...
                if self.Train:
                    ...
                    print(self.list_img_label[15:18])
                    ...
                    print(f'expw_raceds_dict_list_selected: {len(self.expw_raceds_dict_list_selected)}')
...
class EXPWCROPRACE():
    ...
    def get_dataset(self):
        ...
        self.train_ds = DatasetEXPWRACECROP(train= True, 
                                        transform=self.train_transforms,
                                        crop_at_runtime = self.crop_at_runtime)
        
        self.val_ds = DatasetEXPWRACECROP(train = False, 
    ...
    def get_dataloader(self, BATCH_SIZE = None):
        ...
        dataloader_args = dict(shuffle=True, # random info would be provided
                               batch_size=self.BATCH_SIZE,num_workers=4, pin_memory=True) if self.dataconfig.cuda else dict(shuffle=True, batch_size=self.BATCH_SIZE)
...
if __name__ =='__main__':
    ...
    # %%
    import utils
    from ds_expw_crop_race import EXPWCROPRACE
    expw_crop_race_object = EXPWCROPRACE(BATCH_SIZE=6, crop_at_runtime=False)
